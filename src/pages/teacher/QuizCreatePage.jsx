...
import { PlusCircle, Trash2, GripVertical, ImagePlus } from "lucide-react";

export default function QuizCreatePage() {
  const navigate = useNavigate();
  const { createQuiz } = useQuizStore();
  const [step, setStep] = useState(1); // 1: metadata, 2: questions, 3: preview
  ...
  const saveQuestion = async (index) => {
    ...
    try {
      ...
      if (q.id) {
        await api.put(`/questions/${q.id}`, data);
      } else {
        const created = await api.post(`/${quizId}/questions`, data);
        setQuestions((prev) =>
          ...
          <CardContent className="space-y-4 pt-6">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {["NONE", "PER_QUIZ", "PER_QUESTION"].map((t) => (
                  <button
                    key={t}
                    type="button"
      ...
      {questions.map((q, qIndex) => (
        <Card key={qIndex}>
          ...
          <CardContent className="space-y-4">
            {/* Question type */}
            <div className="grid grid-cols-3 gap-2">
              {["MCQ", "TRUE_FALSE", "FILL_BLANK"].map((t) => (
                <button
                  key={t}
                  type="button"
                  onClick={() => {
